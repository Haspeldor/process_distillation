...
\section{Discussion}
% varying bias strength
    % results as expected: the accuracy of enriched model increases, while the accuracy of modified stays the same regardless of how strong 

    % interesting results: base model accuracy also increases in accuracy, similar to enriched model
    % makes sense in the context of process model
    % even when considering that modified should have better accuracy because it can use the gender of patient to predict prostate or mammary screening
    % however, as the bias increases and mostly male patient are classified as allegible for screening 
    % the base model learns to predict prostate screening, since most of the allegible patients are male
    % -> higher accuracy, the higher the bias

% varying number attributes


% varying number of decisions


% tree characteristics
    % unsurprisingly, no changes when varying bias strength
    % 